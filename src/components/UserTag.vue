	...
	<div>
		<div>
			<el-popover class="head" placement="bottom" width="300" trigger="click">
				<div>
					...
					<el-avatar style="margin: 5px;" v-for="n in 9" :key="n" shape="square" :size="50" :src="circleUrl+'0000'+n+'.png'"
					 fit="contain" @click.native="SetUserHeadNO(n)">
					</el-avatar>
				</div>
				<el-avatar slot="reference" shape="square" :size=70 :src="circleUrl+fileName" fit="contain">
				</el-avatar>
			</el-popover>
		...
		<el-collapse class="detailInfo" value="detail">
			<el-collapse-item style="margin-left: 2rem;" title="用户信息" name="detail">
				...
				<li class="detailInfolst">年龄：{{this.userInfo.age}}</li>
				<li class="detailInfolst">经验：{{this.userInfo.experience}}</li>
			</el-collapse-item>
		</el-collapse>
		<el-table :data="tasks" height="200" size="small" border style="width: auto; margin: 0.625rem;">
			<el-table-column prop="taskType" label="类型" width="50">
			</el-table-column>
			<el-table-column prop="createTime" label="创建时间" width="90">
			</el-table-column>
			<el-table-column prop="completeTime" label="完成时间" width="90">
			</el-table-column>
			<el-table-column prop="level" label="维度" width="50">
			</el-table-column>
	...
	export default {
		...
		methods: {
			...
			ViewTask(taskID) {
				...
				console.log("现在TaskID是"+this.$store.state.taskID);
			}
		},
		computed: {
			fileName: function() {
				return this.userInfo.faceID > 9 ? "000" + this.userInfo.faceID + ".png" : "0000" + this.userInfo.faceID + ".png";
			},
			tasks: function() {
